...
function renderFeed() {
  ...
  const filtered = currentFilter === "all" ? allRuns : allRuns.filter((r) => r.event_type === currentFilter);

  feedList.innerHTML = "";

  if (filtered.length === 0) {
    feedList.innerHTML = '<div class="feed-empty">Henüz kayıt yok. Otomasyon ilk çalıştığında burada görünecek.</div>';
    return;
  }
  ...
  filtered.forEach((run) => {
    ...
    card.className = "feed-card" + (run.id === selectedRunId ? " selected" : "");
    card.innerHTML = `
...
function renderDetail(run) {
  ...
  if (!run) {
    panel.innerHTML = '<div class="detail-empty">Bir kart seç, detaylı raporu burada gör.</div>';
    return;
  }
  const meta = EVENT_META[run.event_type] || EVENT_META.degisiklik_yok;
    ...
    keywords.forEach((kw) => {
      ...
      if (d.error) {
        ...
        if (d.new_related && d.new_related.length) {
          ...
          d.new_related.forEach((r) => (bodyHtml += `<li>${escapeHtml(r)}</li>`));
          bodyHtml += `</ul>`;
        }
      }
      bodyHtml += `</div>`;
...
async function openKeywordModal() {
  ...
  const { data, error } = await sb.from("keywords").select("*").order("id", { ascending: true });
  if (error) {
    console.error(error);
    keywordWorkingList = [];
  } else {
    keywordWorkingList = data.map((k) => ({ id: k.id, keyword: k.keyword, active: k.active }));
  }
  renderKeywordList();
}
...
async function saveKeywords() {
  ...
  const keptIds = new Set(keywordWorkingList.filter((k) => k.id).map((k) => k.id));
  ...
  try {
    ...
    for (const k of toUpdate) {
      await sb.from("keywords").update({ keyword: k.keyword, active: k.active }).eq("id", k.id);
    }
    if (toInsert.length) {
      await sb.from("keywords").insert(toInsert.map((k) => ({ keyword: k.keyword, active: k.active })));
    }
    status.textContent = "Kaydedildi ✓";
    setTimeout(() => (document.getElementById("modalOverlay").classList.remove("open")), 600);
  } catch (err) {
    console.error(err);
...
document.getElementById("editKeywordsBtn").addEventListener("click", openKeywordModal);
document.getElementById("closeModalBtn").addEventListener("click", () => {
  document.getElementById("modalOverlay").classList.remove("open");
});
document.getElementById("addKeywordBtn").addEventListener("click", addKeywordFromInput);
